...
## 백엔드 등록 준비 목록 · 14장 (기존 10장 + 추가 승인 4장)

[backend-registration-v1.json](backend-registration-v1.json)은 [catalog-v1.json](catalog-v1.json)의 14장을 검증해 만든 **전달용 payload**다. `suggested_object_key`는 제안 경로이며 실제 S3 업로드·DB 등록 완료 증거가 아니다. 원격 상태는 백엔드 DB와 S3에서 별도로 확인해야 한다.
...
### 2026-09-19 사용자 승인 추가분 · 4장

| asset_key | 최종 PNG | 판결 tag |
|---|---|---|
| `run20260916-mnet-necklace` | [합격 목걸이](run-20260916-new-inputs/002-Mnet-necklace.png) | `APPROVED` |
| `run20260916-proceed` | [진행시켜!](run-20260916-new-inputs/012-images-8.png) | `APPROVED` |
| `run20260916-understood` | [납득해 버렸어!](run-20260916-new-inputs/013-images-9.png) | `NOT_GUILTY` |
| `run20260916-acknowledging-penguin` | [인정 펭귄](run-20260916-new-inputs/014-images-jpeg.png) | `APPROVED` |

인정 펭귄은 사용자 확인으로 분류 검토를 완료했다. 기존 감정 코드 `CELEBRATION`(기쁨·축하·인정)을 사용하고 `유쾌`, `유머러스`를 키워드와 근거에 반영했다. 위 4장은 등록 준비 목록에 포함되었으며, S3·DB 등록이나 활성화 완료를 뜻하지 않는다.

## 신규 변환 결과 · 17장 (4장 등록 준비 목록 반영)

정본은 [manifest.json](run-20260916-new-inputs/manifest.json)과 [annotations.json](run-20260916-new-inputs/annotations.json)이다. 아래 PNG는 각 항목의 **최종본**이며 이미지 옆 JSON은 SHA-256과 감정·키워드를 담는다. `검토 필요`는 생성 실패가 아니라 분류 불확실성이다. 사용자가 승인한 002·012·013·014번은 위 태그를 지정했다. 나머지 13장은 `tag=null`이며, 전체 17장은 운영 등록 전 `is_active=false`를 유지한다.
...
| 002 | [Mnet_그의-손에-쥐어지는-합격-목걸이_고전-밈.webp](<../../inputs/Mnet_그의-손에-쥐어지는-합격-목걸이_고전-밈.webp>) | [002-Mnet-necklace.png](run-20260916-new-inputs/002-Mnet-necklace.png) | [JSON](run-20260916-new-inputs/002-Mnet-necklace.metadata.json) | 완료 · `APPROVED` 승인 |
...
| 012 | [images (8).jpeg](<../../inputs/images (8).jpeg>) | [012-images-8.png](run-20260916-new-inputs/012-images-8.png) | [JSON](run-20260916-new-inputs/012-images-8.metadata.json) | 완료 · `APPROVED` 승인 |
| 013 | [images (9).jpeg](<../../inputs/images (9).jpeg>) | [013-images-9.png](run-20260916-new-inputs/013-images-9.png) | [JSON](run-20260916-new-inputs/013-images-9.metadata.json) | 완료 · `NOT_GUILTY` 승인 |
| 014 | [images.jpeg](<../../inputs/images.jpeg>) | [014-images-jpeg.png](run-20260916-new-inputs/014-images-jpeg.png) | [JSON](run-20260916-new-inputs/014-images-jpeg.metadata.json) | 완료 · `APPROVED` 승인 |
...
1. 남은 분류 `검토 필요` 5장을 사람이 확인하고, 미지정 13장의 판결 `tag` 및 필요한 `strategies`를 승인한다.
2. 등록 준비 목록 14장(기존 10장 + 신규 승인 4장)과 미지정 13장을 구분한다. 파일 이동 시 manifest·metadata 경로 및 SHA-256 연결도 함께 검증한다.
