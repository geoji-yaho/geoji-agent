...
8. **웨이브 종료.** PR 목록과 각 워커의 "확인하지 못한 것"을 사용자에게. **머지는 사용자가 한다.** 다음 웨이브는 머지 뒤 main 에서 다시 2부터
9. **정리.** 워커 워크트리는 PR 머지 뒤 `orca worktree rm`. 사용자가 시킬 때만

## 하지 않는 것

- 코디네이터가 워커의 파일을 직접 고치지 않는다. 지적은 `send --to dispatch:[[ORCA_RICH_MD:866a8f552cdf44ed505e74fd3ea3d1c4:inline-html:%3Cid%3E]]`로 되돌린다
