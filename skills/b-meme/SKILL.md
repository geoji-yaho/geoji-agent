...
감정 6종, 검색 키워드 최대 5개, 지출 카테고리, 대상 종류, 컷별 표정·포즈·자막, 판단 근거와 불확실성을 `annotations.json`에 작성한다. 번들 `scripts/save_metadata.py`를 실행해 허용값을 검증하고 각 이미지 옆 `*.metadata.json`과 manifest에 함께 저장한다. 분류 JSON이 저장되지 않았으면 작업 전체가 완료됐다고 보고하지 않는다. 이미지 생성 성공과 분류 저장 실패를 구분해 알리고, 재개 시 이미지를 다시 만들지 말고 분류 저장만 복구한다.
