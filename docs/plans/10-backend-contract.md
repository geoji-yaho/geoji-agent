...
| `ai-api`·`ai-worker` 이미지 · `docker-compose.prod.yml` | EC2 | 빌드는 GitHub Actions `운영 이미지 빌드`(`.github/workflows/image.yml`, `workflow_dispatch` 에 `ai-YYYYMMDD-N` 입력) → `ghcr.io/geoji-yaho/geoji-agent:<태그>`. EC2 는 `docker pull` 만 한다 — 인스턴스가 메모리 1GB 급이라 거기서 빌드하면 `uv sync` 가 스왑으로 밀려 돌고 있는 `ai-worker` 를 위협한다. GHCR 패키지 public 전환은 조직 정책이 막으므로 `read:packages` PAT 로 `docker login ghcr.io` 한 번 → pull → `.env` 의 `GEOJI_AI_IMAGE` → `docker compose -f docker-compose.prod.yml up -d`. 배포 전 대조는 Actions 마지막 스텝, 배포 뒤는 `docker run --rm <이미지> python -c "from geoji_ai.prompts import prompt_bundle_version as v; print(v())"`. **최신 태그와 대조값은 그 태그를 전달한 문서(19 §8)에 적는다.** |
