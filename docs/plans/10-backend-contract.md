...
|---|---|---|---|
| 9/19 | **카드 본문 상한 30→100자(세 강도 공통, 제목 20자 유지) → 백엔드 코드 변경 없음, AI 이미지만 다시 받아 배포.** 파서는 본문을 1~300 으로 받으므로(server `65bffc2` 대조) 그대로 통과한다. 아래 `ai-20260919-1` 은 이 변경 전 빌드라 **다시 빌드한 태그로 대체**한다. 새 이미지의 `prompt_bundle_version` 은 `bundle-5349a5f3a47c`(서기 v5.8). 카드 화면은 본문이 두세 줄이 될 수 있으니 프론트 렌더만 확인 | §5·§16.3 | 미전달 |
