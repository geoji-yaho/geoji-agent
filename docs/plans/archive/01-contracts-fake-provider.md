...
| (내포) `TextDraft` | | `intensity ∈ mild|spicy|hell`, `headline ≤ 30`, `statement[{text, kind ∈ fact|claim|opinion, evidence_labels[]}]`(1~4항목, 합산 ≤ 300; 9/17 최소 2→1), `banter_strategy`(전략 8종), `selected_candidate_id: UUID | null`, `attack_angle`(서버 지정 6종), `source ∈ AI|TEMPLATE`(**9/11 추가**, 00 §8.3. 서기 strict 에서는 제외). 과거 저장 문구 호환 계약이며 새 생성은 아래 `CardTextDraft` 사용 |
| (생성 전용) `CardTextDraft` | | 9/17: 제목 1~20자, 본문 정확히 1항목·1~30자(**9/19 본문 상한 30→100**, `CARD_STATEMENT_MAX`. 세 강도 공통. 백엔드 파서는 본문을 1~300 으로 받아 변경 없음). 줄바꿈·공백값 거부. 글자 수는 공백·문장부호 포함 code point. 메타데이터는 `TextDraft`와 같으며 `writer_schema`와 워커 파싱·템플릿에서 강제. 백엔드 선반영 지점은 10 §5 |
