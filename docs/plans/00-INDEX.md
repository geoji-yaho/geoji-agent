...
| 미결 | D-23(작업 3 실측 뒤)·검수관 모델(작업 6 실측 뒤)·D-07 팀 비준(M3 검수 시, 기본값 v2 확정). **D-20·D-21·D-22·D-19·D-04·D-08·배포·키·전달·모델은 9/8 확정(§8.4, 10 §15)** |
...
- 성격 요약: **"설계는 두 겹으로 끝났고 코드는 0줄, 백엔드 의존이 임계 경로다."** 작업 1·2 는 우리 혼자 할 수 있지만 M2(작업 3)부터는 백엔드의 내부 API·watchdog·job INSERT 없이는 한 줄도 시연되지 않는다. **9/8 확인: 서버(`geoji-server`)에는 재판 흐름 자체가 없어 10 §2~§9 가 전부 신규다.** D-20 은 Supabase 인스턴스 공유로 확정(10 §15)
...
|:--:|---|:--:|:--:|:--:|:--:|---|
| 1 | ~~D-20 회신~~ **9/8 확정** → 남은 것: `ai` role·grants 생성, Session Pooler 접속 정보 | Critical | 9/9 | — | 10 §15.2 | 백엔드 |
...
```mermaid
flowchart LR
  D10["10 백엔드 계약서 (D-20 9/8 확정)"] -->|"INSERT·API·watchdog"| D03
...
- **9/8 D-08 확정: AI 파트 2명, 일정 그대로.** (참고, 1명일 때의 축소안이었던 것: 07 전체 보류, 06 사람 검수·드립 60 팀 위임, 05 를 9/11 부터 04 와 겹쳐 시작 또는 M3 9/16 — 채택 안 함)
...
| 짤 점수 | **백엔드 finalize**(10 §11) | 08 규칙 제공 | 우리는 `meme_tag`·`hints` + 참조 구현 `domain/meme_selection.py`(9/14 D-28) |
...
- [x] 알림 채널·비용 임계 — 9/8 확정: 디스코드 웹훅, 일 5,000원 경고(10 §15.3)
- [ ] LangSmith·CI 실비(P1)
- [x] 감정 어휘 6종 — 9/8 확정: 08 §3.4 초안 그대로(10 §15.3)
- [ ] 드립 라벨링 담당·검수 인력 3명·짤 제작 담당(10~15장)
- [x] **D-08** — 9/8 확정: AI 파트 2명, 일정 그대로(07 보류·05 조기 착수·M3 이동 없음)
- [ ] 서버 `AiClient` 3종(상 이름·도전 서술·순찰 문구) 동기 엔드포인트 — 9/8 결정 **P1**(09 H)
- [x] **D-24 SENTENCE 게이트** — 9/14 확정: PREPARE 종료 또는 확정 + 30초까지 대기, 마감은 SENTENCE INSERT + 10초(10 §3·§6·§15.5). 백엔드 채택은 10 §14
- [x] **D-25 준비 자료 폴백 계단** — 9/14 확정: 조서+드립 → 조서만 → 즉석 조서(서기·검수·finalize 시간을 남길 때만) → 최소 조서 → 템플릿(10 §15.5, 05 §3.3)
- [x] **D-26 삭제 시 작업 중단** — 9/14 확정: 무효화 트랜잭션에서 진행 중 job `CANCELLED` + 워커 호출 직전 epoch 확인(10 §8·§15.5). 백엔드 채택은 10 §14
- [x] **거부 응답 본문** — 9/14 확정: 양쪽 모두 `{"code"}`(10 §4.7·§15.5)
- [x] **D-27 준비 자료 부분 재사용** — 9/14 확정: 조서·드립(강도별) 부분 키, epoch 변경은 재사용 없음(10 §15.5, 05 §3.2·§3.3)
- [x] **D-28 짤 점수 선택 미정 값** — 9/14 확정: 전략은 `default_intensity` 판결문, 동점 crc32 오름차순, 키워드 NFC·앞뒤 공백 제거 뒤 일치. 실행은 백엔드 finalize, AI 저장소는 참조 구현(10 §11·§15.6, 08 §3.4)
