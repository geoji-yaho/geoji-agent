...
- [ ] **D-20 공유 Postgres**(9/8, 백엔드) — 미회신 시 9/9 저녁 하이브리드 전환 재질문
- [ ] **D-21 강도 enum**·평결 enum 매핑·카테고리 enum(9/9, 백엔드)
- [ ] **D-07 지옥맛 `guardrail-v2` 비준**(팀, M3 검수 시) — 미비준 시 `guardrail-v1` 전환
- [ ] **D-19 양형 이유 템플릿 치환**(팀 확인) — 계획서 기본값은 치환
- [ ] **D-22 모델 동시성 8**(벤더 rate limit 확인, 실측에서 429 시 하향)
- [ ] **D-23 양형관 실측 > 2초 대응** — 작업 3 실측 결과 기록 자리: sentencing p90 ____초 → 유지 / 밴드별 사전 후보(09 G-8)
- [ ] **D-04 댓글 메모리 고지**(팀) — 확정 전 `ROOM_COMMENT_STYLE_ENABLED=false`
...
- [ ] 배포 구조(ai-api·ai-worker Compose), xAI·OpenAI 키·결제 주체, 알림 채널·비용 임계, LangSmith·CI 실비
- [ ] 드립 라벨링 담당·검수 인력 3명·짤 제작 담당(10~15장)·감정 어휘 6종
- [ ] **D-08 일정 재산정** — §7 부하 표(AI 2.2명, 백엔드 7~9 인일) 기준으로 07 보류·05 조기 착수·M3 이동 여부
