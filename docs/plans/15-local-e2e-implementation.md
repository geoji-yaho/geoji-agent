...
| 13 (9/19) | 핸드오프 추적 `scripts/trace_pipeline.py --execute`(택시 단골 사건, 실제 모델 10회, `AI_READY`, 재작성 1) | ① **심문관 후처리가 `schema_version` 을 안 채워 실제 호출이 전부 `INVALID_OUTPUT`→FALLBACK**(strict 스키마엔 그 키가 없다). ② 서기가 드립 후보 5개를 하나도 안 골랐다(`selected_candidate_id` 전부 null) — 후보가 60~90자 두세 문장이라 30자 카드에 못 쓴다. ③ 검수관이 지옥맛 "카드 부검당한다" 를 `SELF_HARM_LEXICON` 으로 반려 → 재작성. 서기 프롬프트는 부검을 허용하고 검수관 예외는 '사망 선고' 하나뿐(6회차 미해결분 재현). ④ 세 강도가 같은 각도(미래 예언)라 mild·spicy 문구가 거의 같다. ⑤ 조서 LLM 추론 F6·F7 은 F3·F1 을 되풀이한 문장이고 "예산 소진율 41%" 가 MITIGATION 으로 붙었다. ⑥ 심문관 결과·조서 사유 분석은 어느 프롬프트에도 안 간다 | ① `validate_intake_output` 에 `schema_version` 채움(PR #50). ② `banter-v2.md` 카드 길이 + 서버 필터 5규칙(`CARD_STATEMENT_MAX` 초과 삭제). ③ `guardrail-v2.md` 돈 주어 블랙 코미디 통과 명시(②·③ 9/19, 14회차). ④·⑤·⑥ 은 `test_gap_*` 로 고정해 두고 결정 대기: 강도별 각도 분리(05), 조서 추론 중복 제거(05), 심문·사유 분석 전달(05) |
| 14 (9/19) | ②·③ 적용 뒤 같은 추적 지옥맛 방 1회(실제 모델 10회, `AI_READY`, 재작성 1. 카드 상한 30→100 이전, 서기 v5.7) | 드립 후보 4개가 17~24자로 나와 필터를 다 통과했고 서기 셋에 그대로 전달됐다. 지옥맛 "크리스마스에 통장 장례 치르겠네" 를 검수관이 통과시켰다(③ 확인). 반려 1건은 매운맛 제목 "연말엔 발이 잘릴 거다"(신체 훼손) 로 정당한 반려, 재작성 통과. `selected_candidate_id` 는 여전히 전부 null — 서기 프롬프트가 후보를 "참고만, 복사 금지" 로 두고 각도(미래 예언)가 후보 전략과 안 맞아서다. 세 강도 같은 각도(④) 재현 | 후보 활용은 서기 프롬프트 문제(v5.8 후보, 06). ④ 는 결정 대기 |
| 12 (9/18) | 점검 도구 사건 6개로 확대 뒤 실측 4회(12건, 형식 11/12 통과) | 첫 지출(최소 조서, F0 만)인데 해시가 반복·규칙 의인화 각도면 서기가 지어낸다: "42번째 키보드", "33번째 펌?", "규칙이 절교 선언"(무죄·기각 사건 포함). 검수관 `UNGROUNDED_CLAIM` 반려 → 재작성·템플릿으로 가는 경로. 형식 실패 1건은 `banter_strategy` 에 각도 이름을 넣은 enum 위반(xAI strict 가 enum 도 강제하지 않음) | `pick(..., skip=ungrounded_angles(dossier))`: 조서에 과거 기록·반복 집계·규칙 근거가 없으면 그 각도를 건너뛴다(01 §3.5·05 §3.3). 배심원 투표 사유는 스냅샷에 없어 10 §0.1·§14 에 요청 |
...
- 판결문에 티어 코드가 영어 그대로 샌다("king 티어"). 8·9회차와 #72 운영 화면에서 확인했다.
  9/23 #72: 화면의 피고인 배지는 이미 "무일푼"이며 AI 제목·본문이 `penniless`를 인용했다.
  원인은 `build_evidence._aggregates`가 백엔드 enum을 그대로 근거 문장에 넣는 경로다.
  [프론트 정본](https://github.com/geoji-yaho/geoji-web/blob/main/src/shared/domain/tier.ts)의
  `king→거지왕`, `flower→꽃거지`, `hardcore→상거지`, `penniless→무일푼`을 새 집계 근거에 적용했다.
  이미 한글인 값은 유지하고 미등록 값은 티어 구절만 생략한다. 예산·무지출·반복 집계는 유지한다.
  재사용 조서도 조서·드립·양형·서기·검수 모델의 근거 입력에서 같은 표시명을 사용한다.
  변환은 DB 집계의 고정 `티어 <코드>,` 구절에 한정하며 사용자 원문·저장된 근거·enum 계약은 바꾸지 않는다.
  프롬프트 변경·추가 모델 호출은 없다. 새 근거와 캐시 조서 경로는 fake 회귀로 검증했다.
  기존 저장 판결과 과거 드립·모델 추론 문구의 일괄 수정은 하지 않는다. 실제 모델·운영 화면 재검증은 남아 있다.
