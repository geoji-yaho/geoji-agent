...
| (생성 전용) `CardTextDraft` | | 9/17: 제목 1~20자, 본문 정확히 1항목·1~30자. 줄바꿈·공백값 거부. 글자 수는 공백·문장부호 포함 code point. 메타데이터는 `TextDraft`와 같으며 `writer_schema`와 워커 파싱·템플릿에서 강제. 백엔드 선반영 지점은 10 §5 |
