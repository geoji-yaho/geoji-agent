...
| `writer` (fan-out) | 강도마다 `asyncio.create_task`(세마포어 8 안): 입력 = 양형 결과(인용·수정 금지), Evidence(라벨), 그 강도 후보(`fits ∋ result`), 그 강도 말투 예시(플래그), `attack_angle = angles.pick(post_id, offset, skip=ungrounded_angles(dossier))`(9/18: 조서에 이력·규칙 근거가 없으면 반복·규칙 의인화를 건너뛴다. 01 §3.5), **그 강도 섹션만의 시스템 프롬프트**. 출력 `TextDraft` 1개 + `meme_tag`·`meme_hints`(`default_intensity` 호출 값 채택). 실패 강도는 `draft_sources[i]=TEMPLATE`(`templates-v1.json`) |
...
**9/20 반복 반려 수정(21 §3):** 새 작성 시도의 캐시는 job·mode·round·call_index로 분리한다. 검수에는 기존 라벨→텍스트와 함께 `evidence_metadata`의 epistemic_type·fact_type을 전달한다. 정상적인 무형량 사건은 LLM 출력에서 형량 검사를 제외하고 코드가 적용 제외 결과를 채운다. 형량이 필요한 사건의 누락과 실제 문구 검수 실패는 그대로 차단한다. 같은 작업의 보정에는 위반 설명·경로·근거 라벨도 전달한다. TEXT_RETRY 작업 간 실패 상세 전달은 현재 계약에 없으므로 포함하지 않는다. 구현·검증 상태는 21 §5를 따른다.
...
**9/19 — 본문 상한 30→100자.** 30자는 세 강도 모두 판결문이 너무 짧다는 사용자 결정으로 100자로 올렸다
(01 `CardTextDraft.CARD_STATEMENT_MAX`, 제목 20자는 그대로). 위 1~2 의 구제·재작성 조건과 메시지는 상수만
따라가고 경계가 100 이 된다. 재작성 지시는 "한 문장" 에서 "두 문장 이내" 로 바뀐다. 서기 프롬프트는 v5.8
(06 §3.3, 최대 두 문장·목표 40~70자). 백엔드 `FinalizeRequestParser` 는 본문을 1~300 으로 받으므로 백엔드
변경은 없고 새 이미지만 필요하다(10 §0.1). 실호출 회귀·사람 검수 전.
