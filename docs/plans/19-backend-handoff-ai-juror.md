...
- 프롬프트 파일이 늘어 `prompt_bundle_version` 이 바뀐다. 운영 이미지 태그와 대조값: **`ai-20260921-1` = `bundle-8e868116333c`**(9/21 배포, main `0984ea0`). 직전은 `ai-20260919-2` = `bundle-afa59044c8df`. 대조는 `docker run --rm <이미지> python -c "from geoji_ai.prompts import prompt_bundle_version as v; print(v())"` 또는 워커 로그 `sentence_call`·`sentence_fallback` 의 `prompt_bundle_version` — `sentence_summary` 에는 이 필드가 없다(`application/sentence_case.py`)
