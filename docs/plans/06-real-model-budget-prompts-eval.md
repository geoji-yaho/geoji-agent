...
| `banter-v2.md` | **(9/19 적용, `BANTER_PROMPT`)** v1 + 카드 길이(text 1~30자 한 문장, 목표 15~22자, 글자 수 적은 예시 2개), 후보 간 소재·기법 중복 금지, 승인 예시 참고·복사 금지, 지옥맛 허용 목록 삭제(9/16 결정을 드립에도). 이유: 실제 모델 추적에서 60~90자 후보 5개를 서기가 하나도 안 골랐다(15 §6 13회차). 서버 필터 5규칙(05 §3.2)이 30자 초과를 버린다. 적용 뒤 추적 1회: 후보 4개 17~24자(15 §6 14회차). **골든셋 회귀·사람 검수 전** |
